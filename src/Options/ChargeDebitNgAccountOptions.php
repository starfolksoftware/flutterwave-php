...
final class ChargeDebitNgAccountOptions extends OptionsAbstract
{
    /**
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->define('account_bank')
            ->required()
            ->allowedTypes('string')
            ->info('The name of the bank where the account is held.');

        $resolver->define('account_number')
            ->required()
            ->allowedTypes('string')
            ->info('The account number');
